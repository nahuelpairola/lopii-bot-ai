...
| `quote` | Model + repository + `net/http` client for two public series: daily USD rates (`usd_quotes`, one row per date+casa) and monthly CPI (`monthly_cpi`). Write-only so far — ingestion for a later Mini App consumption stage |

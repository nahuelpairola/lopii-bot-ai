...
| `internal/movement/` | `repository.go`, `last_transaction_store.go`, `messages.go` | `Movement` model (incl. `Subcategory *subcategory.Subcategory` GORM association, `foreignKey:SubcategoryID`) + repository (`InsertBatch`, `FindSimilarForUser` (pg_trgm fuzzy search, `until *time.Time` optional upper date bound, `Preload("Subcategory")`), `SoftDeleteByIDs`, `ReplaceMovements`, `SumAmountForAccount`) + `LastTransactionStore` (in-memory, per-user, most-recent transaction group) + `IconForType`/`TypeFromString` helpers |

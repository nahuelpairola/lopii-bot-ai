...
- **Onboarding accounts are created atomically with opening movements.** `motion.InsertAccountsWithOpenings` (a cross-repo method wrapping both `account` and `movement` tables in a single `gorm.DB.Transaction`) inserts N accounts + N opening `transfer` movements in one shot: a mid-insert failure rolls back all, so onboarding never leaves a user with half their accounts. Opening balance is captured in the first movement; later corrections/additions use the standard CREATE/UPDATE/DELETE flows.

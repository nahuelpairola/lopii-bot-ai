...
- Implicit references ("actually it was 1200") resolve via `resolveCandidates` (in-Go scoring over a DB window: the most recent `recencyLimit` movements by entry time `created_at`, capped at 90 days; a mentioned date anchors on business `date` instead) — no in-memory last-transaction store

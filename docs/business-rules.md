...
- **A message that names the account and states the total goes straight to the confirm screen** ("Mercado Pago a $207555,66"): no picker, no menu, no retyping. With the account but no total it asks only for the total. An account name held in both currencies resolves to USD when the message says so (`usd`, `u$s`, `us$`, `dólares`, `verdes`) and to ARS otherwise. The confirm screen always offers to change the amount or the account.

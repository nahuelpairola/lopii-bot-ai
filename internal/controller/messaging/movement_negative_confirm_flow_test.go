...
func TestNegativeConfirm_RegisterIgualInserts(t *testing.T) {
	...
	data := conversation.Data{conversation.UserIDKey: uint64(1), "mode": "create", "movements": encodeMovementRows(rows), "old_movement_ids": encodeStringSlice(nil), "_gate_choice": "register"}

	c.finishMovementNegativeConfirmFlow(context.Background(), nil, 0, data)

...
Clasificá el mensaje del usuario en una de estas 9 acciones:
...
- CATEGORY_MANAGE: el mensaje pide sacar, borrar o unificar una categoría o subcategoría QUE YA EXISTE. Señal clave: habla de deshacerse de una clasificación o de que dos son la misma, no de crear una nueva ni de clasificar un movimiento. Ejemplos: "borrá la categoría mascotas", "tengo dos categorías repetidas", "sacá la subcategoría delivery", "comida y alimentos son lo mismo".
...
var routerTool = toolSchema{
	...
	Parameters: json.RawMessage(`{
		"type": "object",
		"properties": {
			"intent": {"type": "string", "enum": ["CREATE", "UPDATE", "DELETE", "QUERY", "ACCOUNT_MANAGE", "CREATE_CATEGORY", "CATEGORY_MANAGE", "REMINDER_SET", "HELP"]},
...
func (o *Orchestrator) ClassifyIntent(ctx context.Context, text string) (IntentResult, error) {
	...
	switch args.Intent {
	case IntentCreate, IntentUpdate, IntentDelete, IntentQuery, IntentAccountManage, IntentCreateCategory, IntentCategoryManage, IntentReminderSet, IntentHelp:
